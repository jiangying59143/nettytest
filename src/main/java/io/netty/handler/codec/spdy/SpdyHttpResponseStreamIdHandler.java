...
/**
 * {@link MessageToMessageCodec} that takes care of adding the right {@link Names#STREAM_ID} to the
...
public class SpdyHttpResponseStreamIdHandler extends
    ...
    protected void encode(ChannelHandlerContext ctx, HttpMessage msg, List<Object> out) throws Exception {
        Integer id = ids.poll();
        if (id != null && id.intValue() != NO_ID && !msg.headers().contains(Names.STREAM_ID)) {
